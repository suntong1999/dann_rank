...
class ChiefLastExitHook(session_run_hook.SessionRunHook):
  ...
  def begin(self):
    """Count the number of workers and chief, and
       setup exit counter queue
    """
    ...
    with tf.device(tf.DeviceSpec(job='ps', task=0,
      ...
      self._queue = tf.FIFOQueue(capacity=self._num_worker,
                                 ...
                                 name='worker_exit_counter',
                                 shared_name='worker_exit_counter')
  ...
  def after_create_session(self, session, coord):
    """Clean up the queue, as there are sometimes ps is not
       exit, the last run enqueued elements will remain
       in the queue
    """
    if self._is_chief:
  ...
  def end(self, session):
    """Only when all workers and chief enqueue an
       element, will the end method exit
    """
